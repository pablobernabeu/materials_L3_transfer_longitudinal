...
Session3_Experiment_gender_agreement = 
  ...
  function(stimuli, study_site, language, verbose = FALSE) {
    ...
    # used in Session 2 are used in Session 3 for the content on gender 
    # agreement. In this way, the nouns are rotated across the grammatical 
    # properties and the sessions.
    ...
    combinations %>% 
      ...
      write.csv(paste0(path, study_site, ' site, ', language, ', ',
                       'Session3_Experiment_gender_agreement, List 1.csv'), 
    ...
    combinations %>% 
      filter(list == combinations %>% 
               ...
               list == '') %>%
      write.csv(paste0(path, study_site, ' site, ', language, ', ',
                       'Session3_Experiment_gender_agreement, List 2.csv'), 
                row.names = FALSE, fileEncoding = 'UTF-8')
    ...
    combinations %>% 
      filter(list == combinations %>% 
               ...
               list == '') %>%
      write.csv(paste0(path, study_site, ' site, ', language, ', ',
                       'Session3_Experiment_gender_agreement, List 3.csv'), 
                row.names = FALSE, fileEncoding = 'UTF-8')
