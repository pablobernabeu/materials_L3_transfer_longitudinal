...
# Creation of stimuli for the test administered after the training in differential 
# object marking (DOM). Participants can only move on to the experiment if they 
# score above 80%, as in González Alonso et al. (2020; 
# https://doi.org/10.1016/j.jneuroling.2020.100939).

# Example sentence structure: 'Amelia chose fi ze truck'
...
# 5. semantic violation (i.e., noun different from image)

# In all conditions except 'number violation', the articles are in 
# singular because using plural would require verb-object number 
...
Session3_Test_differential_object_marking = 
  ...
  function(stimuli, study_site, language) {
    ...
    combinations$sentence_DOM_violation = NA
    ...
    for(i in 1:nrow(combinations)) {
      ...
      combinations[i,] = combinations[i,] %>%
        ...
        mutate( number = 'singular',  # modified below in violation conditions
                ...
                correct_sentence =
                  ifelse(language == 'Mini-Norwegian',
                         paste0(person, ' ', verb, ' ', DOM_morpheme, ' ', paste0(noun, article), '.'),
                         ifelse(language == 'Mini-English' | language == 'Mini-Spanish',
                                paste0(person, ' ', verb, ' ', DOM_morpheme, ' ', article, ' ', noun, '.'),
                                NA)) %>% 
                  str_to_sentence(),  # Make first letter in sentence uppercase
                
                sentence_DOM_violation =
                  ifelse(language == 'Mini-Norwegian',
                         paste0(person, ' ', verb, ' ', paste0(noun, article), '.'),
                         ifelse(language == 'Mini-English' | language == 'Mini-Spanish',
                                paste0(person, ' ', verb, ' ', article, ' ', noun, '.'),
                                NA)) %>% 
                  str_to_sentence(),  # Make first letter in sentence uppercase
                
                sentence_number_violation =
                  ifelse(language == 'Mini-Norwegian',
                         paste0(person, ' ', verb, ' ', DOM_morpheme, ' ', paste0(noun, article_number_violation), '.'),
                         ifelse(language == 'Mini-English' | language == 'Mini-Spanish',
                                paste0(person, ' ', verb, ' ', DOM_morpheme, ' ', article_number_violation, ' ', noun, '.'),
                                NA)) %>% 
                  str_to_sentence(),  # Make first letter in sentence uppercase
                ...
                sentence_semantic_violation =
                  ifelse(language == 'Mini-Norwegian',
                         paste0(person, ' ', verb, ' ', DOM_morpheme, ' ', paste0(noun_semantic_violation, article_semantic_violation), '.'),
                         ifelse(language == 'Mini-English' | language == 'Mini-Spanish',
                                paste0(person, ' ', verb, ' ', DOM_morpheme, ' ', article_semantic_violation, ' ', noun_semantic_violation, '.'),
                                NA)) %>% 
                  str_to_sentence()  # Make first letter in sentence uppercase
        )
      ...
      for(i in 1:nrow(combinations)) {
        ...
        combinations[i,] = combinations[i,] %>%
          mutate( verb_image_1 = paste0(i_verb_label, '_1', '.png'),
                  ...
                  noun_image = paste0(i_noun_label, '.png') )
      }
    }
    ...
    combinations %>% 
      filter(list == combinations %>% 
               ...
               list == '') %>%
      write.csv(paste0('session_materials/Session 3/stimuli/test/', 
                       study_site, ' site, ', language, ', ',
                       'Session3_Test_differential_object_marking, List 1.csv'), 
    ...
    combinations %>% 
      ...
      write.csv(paste0('session_materials/Session 3/stimuli/test/', 
                       study_site, ' site, ', language, ', ',
                       'Session3_Test_differential_object_marking, List 2.csv'), 
                row.names = FALSE, fileEncoding = 'UTF-8')
