...
# Creation of stimuli for the training part of Session 4 (verb-object agreement)
...
Session4_Training_verb_object_agreement = 
  ...
  function(stimuli, study_site, language) {
    ...
    combinations = stimuli %>%
      ...
      group_by(noun1) %>%
      mutate(noun1_number = rep(c('plural', 'singular'), times = n()/2)) %>%
      ungroup()
    ...
    # Select article based on the gender and number of noun1.
    ...
    # The morphemes of verb-object agreement and differential object marking are 
    # inserted at this point. 
    
    DOM_morpheme = stimuli[stimuli$DOM_morpheme != '', 'DOM_morpheme']
    VOA_morpheme = stimuli[stimuli$VOA_morpheme != '', 'VOA_morpheme']
    
    combinations$VOA_morpheme1 = ''
    combinations$VOA_morpheme2 = ''
    ...
    for(i in 1:nrow(combinations)) {
      ...
      combinations[i,] = combinations[i,] %>% mutate( 
        ...
        VOA_morpheme1 = case_when( noun1_number == 'plural' & 
                                     str_detect(verb1, '[aeiou]$') ~ VOA_morpheme,
                                   # Add epenthetic vowel 'e' when verb ends in consonant
                                   noun1_number == 'plural' & 
                                     str_detect(verb1, '[^aeiou]$') ~ paste0('e', VOA_morpheme), 
                                   .default = '' ),
        
        VOA_morpheme2 = case_when( noun1_number == 'plural' & 
                                     str_detect(verb2, '[aeiou]$') ~ VOA_morpheme,
                                   # Add epenthetic vowel 'e' when verb ends in consonant
                                   noun1_number == 'plural' & 
                                     str_detect(verb2, '[^aeiou]$') ~ paste0('e', VOA_morpheme), 
                                   .default = VOA_morpheme2 ),
        
        sentence1 = ifelse( language == 'Mini-Norwegian',
                            paste0(person, ' ', paste0(verb1, VOA_morpheme1), ' ', 
                                   DOM_morpheme, ' ', noun1_with_article, '.'),
                            ifelse(language == 'Mini-English' | language == 'Mini-Spanish',
                                   paste0(person, ' ', paste0(verb1, VOA_morpheme1), ' ', 
                                          DOM_morpheme, ' ', article, ' ', noun1, '.' ),
                                   NA)) %>% 
          str_to_sentence(),  # Make first letter in sentence uppercase
        
        sentence2 = ifelse( language == 'Mini-Norwegian',
                            paste0(person, ' ', paste0(verb2, VOA_morpheme2), ' ', 
                                   DOM_morpheme, ' ', noun1_with_article, '.'),
                            ifelse(language == 'Mini-English' | language == 'Mini-Spanish',
                                   paste0(person, ' ', paste0(verb2, VOA_morpheme2), ' ', 
                                          DOM_morpheme, ' ', article, ' ', noun1, '.' ),
    ...
    if(language == 'Mini-English') {
      ...
      for(i in 1:nrow(combinations)) {
        ...
        if(combinations[i, 'noun2'] == '') {
          ...
          combinations[i,] = combinations[i,] %>% mutate( 
            verb1_image1 = paste0(verb1, '_1', '.png'), 
          ...
          # If plural, compose image for noun1
          if(combinations[i, 'number_of_items_in_noun1'] > 1) {
            
            noun_image = 
              paste0(image_dir, combinations[i, 'unprocessed_noun1'], '.png') %>% 
            ...
            if(combinations[i, 'number_of_items_in_noun1'] == 2) {
              noun_image = 
                image_read(paste0(image_dir, 'background.png')) %>%
                image_composite(image_append(c(noun_image, noun_image)) %>% 
                                  image_scale('720x720') %>% 
                                  image_convert(colorspace = 'sRGB'),  # preserve colours
                                gravity = 'center')
              
            } else if(combinations[i, 'number_of_items_in_noun1'] == 3) {
              noun_image = 
                image_read(paste0(image_dir, 'background.png')) %>%
                image_composite(image_append(c(noun_image, noun_image, noun_image)) %>% 
                                  image_scale('720x720') %>% 
                                  image_convert(colorspace = 'sRGB'),  # preserve colours
                                gravity = 'center')
            }
            ...
            # Save noun_image
            image_write(noun_image,
                        paste0(image_dir,
                               combinations[i, 'unprocessed_noun1'], 
                               ifelse(combinations[i, 'number_of_items_in_noun1'] == 1, '', 
                                      paste0('_', combinations[i, 'number_of_items_in_noun1'])),
                               '.png'),
                        format = 'PNG')
          }
      ...
      for(i in 1:nrow(combinations)) {
        ...
        if(combinations[i, 'noun2'] == '') {
          ...
          combinations[i,] = combinations[i,] %>% mutate( 
            ...
            verb1_image1 = paste0(i_verb1_label, '_1', '.png'),
    ...
    # Clean up multiple NA images
    unlink(paste0(image_dir, 'NA_*.png'))
    ...
    combinations %>%
      ...
      select(materials_version, person, verb1_image1, verb1_image2, verb1_image3, 
             verb1_image, verb2_image1, verb2_image2, verb2_image3, verb2_image, 
             noun_image, upper_image1, lower_image1, upper_image2, lower_image2, 
             sentence1, sentence2) %>%
      
      # Save
      write.csv(paste0('session_materials/Session 4/stimuli/training/', 
                       study_site, ' site, ', language, ', ',
                       'Session4_Training_verb_object_agreement.csv'), 
