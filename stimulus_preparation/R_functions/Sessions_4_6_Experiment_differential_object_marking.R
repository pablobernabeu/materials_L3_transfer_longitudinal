...
# marking administered in Sessions 4 and 6.
...
# agreement, which is implemented in the set of trials corresponding 
# to VOA (see script 'Sessions_4_6_Experiment_verb_object_agreement.R').
  ...
  function(stimuli, study_site, language, experiment_path, verbose = FALSE) {
    ...
    for(i in 1:nrow(combinations)) {
      ...
      if(combinations[i, 'wrapup_format'] == 'additive') {
        ...
        if(language == 'Mini-Norwegian') {  # for Mini-Norwegian only
          ...
          # Time-lock triggers to the onset of the first word 
          # after the DOM morpheme.
    ...
    combinations %>% 
      ...
      write.csv(paste0(experiment_path, study_site, ' site, ', language, ', ',
                       'Sessions_4_6_Experiment_differential_object_marking, List 1.csv'), 
    ...
    combinations %>% 
      filter(list == combinations %>% 
               ...
               list == '') %>%
      write.csv(paste0(experiment_path, study_site, ' site, ', language, ', ',
                       'Sessions_4_6_Experiment_differential_object_marking, List 2.csv'), 
                row.names = FALSE, fileEncoding = 'UTF-8')
    ...
    combinations %>% 
      filter(list == combinations %>% 
               ...
               list == '') %>%
      write.csv(paste0(experiment_path, study_site, ' site, ', language, ', ',
                       'Sessions_4_6_Experiment_differential_object_marking, List 3.csv'), 
                row.names = FALSE, fileEncoding = 'UTF-8')
